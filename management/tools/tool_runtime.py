...
Local-first tools work without internet; external tools fail closed when their
credentials/connectivity are unavailable.
"""
...
class ToolRuntime:
    ...
    def list_tools(self) -> list[dict[str, Any]]:
        return [
            {"name": s.name, "category": s.category, "description": s.description, "offline": s.offline}
    ...
    def execute(self, name: str, **kwargs: Any) -> ToolResult:
        ...
        except Exception as exc:  # tool boundary: never crash the agent runtime
    ...
    def _run(self, command: str, timeout: int = 120) -> subprocess.CompletedProcess[str]:
        return subprocess.run(command, cwd=self.root_dir, shell=True, text=True, capture_output=True, timeout=max(1, min(timeout, 600)))
    ...
    def _git_status(self, **_: Any) -> ToolResult:
        return self._command_run("git status --short")

    def _git_diff(self, **_: Any) -> ToolResult:
        return self._command_run("git diff --")

    def _git_log(self, limit: int = 10, **_: Any) -> ToolResult:
        return self._command_run(f"git log --oneline -n {max(1, min(limit, 50))}")

    def _git_branch(self, name: str, **_: Any) -> ToolResult:
        return self._command_run(f"git switch -c {name}")

    def _git_commit(self, message: str, **_: Any) -> ToolResult:
        return self._command_run(f'git add -A && git commit -m "{message.replace(chr(34), chr(39))}"')

    def _package_install(self, manager: str, package: str, **_: Any) -> ToolResult:
        if manager not in {"npm", "pip"}:
            return ToolResult("package.install", False, error="Only npm and pip are supported")
        cmd = f"{manager} install {package}" if manager == "npm" else f"python -m pip install {package}"
        return self._command_run(cmd, timeout=600)
    ...
    def _website_status(self, url: str, **_: Any) -> ToolResult:
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as r:
                return ToolResult("website.status", True, {"status": r.status, "url": r.url})
        except urllib.error.HTTPError as e:
            return ToolResult("website.status", True, {"status": e.code, "url": url})
    ...
    def _api_request(self, url: str, method: str = "GET", body: Any = None, headers: Dict[str, str] | None = None, **_: Any) -> ToolResult:
        ...
        with urllib.request.urlopen(req, timeout=30) as r:
            return ToolResult("api.request", True, r.read(50000).decode("utf-8", errors="replace"), {"status": r.status})

    def _github_request(self, path: str, **_: Any) -> ToolResult:
        token = os.getenv("GITHUB_TOKEN")
        if not token:
            return ToolResult("github.request", False, error="GITHUB_TOKEN is not configured")
    ...
    def _gemini_request(self, prompt: str, **_: Any) -> ToolResult:
        ...
        if not key:
            return ToolResult("gemini.request", False, error="GOOGLE_API_KEY is not configured")
    ...
    def _security_secrets(self, **_: Any) -> ToolResult:
        ...
        for p in self.root_dir.rglob("*"):
            if not p.is_file() or any(x in p.parts for x in {"node_modules", ".git", "dist", "build"}):
                continue
            try:
            ...
            for marker in patterns:
                if marker in text:
                    hits.append({"file": str(p.relative_to(self.root_dir)), "pattern": marker})
        return ToolResult("security.secrets", not hits, {"findings": hits})

    def _security_dependencies(self, **_: Any) -> ToolResult:
        if not (self.root_dir / "package-lock.json").exists():
            return ToolResult("security.dependencies", True, {"skipped": "No package-lock.json"})
    ...
    def _not_configured(self, **_: Any) -> ToolResult:
        return ToolResult("business.adapter", False, error="Adapter contract registered; external business system is not configured")

    def _media_script(self, title: str, outline: list[str] | None = None, **_: Any) -> ToolResult:
        return ToolResult("media.script", True, {"title": title, "outline": outline or [], "status": "draft_spec"})

    def _media_job(self, title: str, **_: Any) -> ToolResult:
        return ToolResult("media.job", True, {"title": title, "status": "job_spec_created"})


if __name__ == "__main__":
    runtime = ToolRuntime()
    print(json.dumps(runtime.list_tools(), indent=2))
