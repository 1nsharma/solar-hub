...
export const useStore = create((set) => ({
  ...
  fetchProducts: async () => {
    try {
      ...
      if (data && data.products && data.products.length > 0) {
        set({ products: data.products, services: data.services || FALLBACK_SERVICES });
      }
    } catch (err) {
      console.warn('API unavailable, keeping fallback catalogue:', err);
    }
  },
  ...
  addOrder: async (order) => {
    try {
      ...
      throw new Error('Order API responded with error');
    } catch (err) {
      console.warn('API unavailable, adding order locally:', err);
      const localOrder = {
        id: `ord_${Date.now()}`,
        ...order,
  ...
  addBooking: async (booking) => {
    try {
      ...
      throw new Error('Booking API responded with error');
    } catch (err) {
      console.warn('API unavailable, adding booking locally:', err);
      const localBooking = {
        id: `book_${Date.now()}`,
        ...booking,
        created_at: new Date().toISOString(),
      ...
      set((state) => ({ bookings: [localBooking, ...state.bookings] }));
      return localBooking;
    }
  },
